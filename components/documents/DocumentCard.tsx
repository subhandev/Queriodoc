...
import { libraryBanner, sampleCard } from "@/lib/onboarding/copy";
...
export function DocumentCard({ document, onDelete }: DocumentCardProps) {
  ...
  const chatHref = `/documents/${document.id}`;
  const isReady = document.status === "ready";
  const isSample = document.is_sample === true;
  const ctaLabel = isSample ? libraryBanner.cta : "Open chat";
    ...
    <>
      {isReady ? (
        <Link
          ...
          className="group relative flex flex-col rounded-xl border border-border bg-card p-5 transition-all duration-150 hover:border-[rgba(255,255,255,0.15)] hover:bg-[#1C1C22] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
        >
          {cardInner}
        </Link>
      ) : (
        <div
          className="group relative flex flex-col rounded-xl border border-border bg-card p-5 opacity-70"
